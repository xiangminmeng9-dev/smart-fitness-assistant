...
async def generate_fitness_plan(
    ...
    try:
        ...
        urgency_desc = "高" if weight_urgency > 0.5 else "中" if weight_urgency > 0.2 else "低"

        # Compact prompt — only ask AI for workout + recommendations
        # Meal plan is generated by our code (faster & more consistent)
        prompt = f"""专业健身教练，为用户生成今日训练计划（纯JSON，无markdown）。

用户：{"男" if gender == "male" else "女"}，{age}岁，{height}cm，{weight}kg→{target_weight}kg（{"减" if weight_diff > 0 else "增"}{abs_weight_diff}kg），{goal}，每周{frequency}次
BMR:{targets['bmr']} TDEE:{targets['tdee']} 蛋白质:{targets['protein_g']}g 紧迫度:{urgency_desc}
周期第{cycle_info['current_day']}/{cycle_info['total_days']}天，强度:{cycle_info.get('intensity', 'medium')}
今日：{"休息日" if is_rest_day else muscle_str}

返回JSON：
{{"motivation_quote":"激励语","warmup":["动作1","动作2","动作3"],"workout_groups":[{{"muscle_group":"肌群名","exercises":[{{"name":"动作","sets":4,"reps":"8-12","rest_seconds":90,"weight_suggestion":"建议","calories_burned":80,"notes":"要点","demo_url":"B站视频链接","exercise_completed":false}}]}}],"cooldown":["拉伸1","拉伸2"],"recommendations":["建议1含抗衰老/体态/护肤","建议2"]}}

要求：每个肌群3-5个动作，demo_url填真实B站搜索链接，recommendations含1条抗衰老建议"""

        provider = get_ai_provider(provider_type, base_url, api_key)
        # AI generates workout + recommendations only (meal plan is local)
        # max_tokens=1500 keeps response under 15s for fast models
        try:
            import asyncio
            response_text = await asyncio.wait_for(
                provider.generate(prompt, model_name, max_tokens=1500),
                timeout=30.0
            )
        except asyncio.TimeoutError:
            print(f"[AI] AI call timed out (30s), falling back to mock plan")
            return generate_mock_plan(profile, plan_date, muscle_groups, is_rest_day, context, weather_info)
            ...
            if "401" in err_str or "404" in err_str or "403" in err_str:
                ...
                try:
                    response_text = await asyncio.wait_for(
                        get_ai_provider(alt_type, base_url, api_key).generate(prompt, model_name, max_tokens=1500),
                        timeout=25.0
                    )
        ...
        try:
            ...
            print(f"[AI] Failed to parse AI JSON, falling back to mock. Raw: {response_text[:200]}")
            return generate_mock_plan(profile, plan_date, muscle_groups, is_rest_day, context, weather_info)

        # Merge AI workout with locally-generated meal plan
        total_exercise_calories = 0
        for g in ai_plan.get("workout_groups", []):
            for ex in g.get("exercises", []):
                total_exercise_calories += ex.get("calories_burned", 50)
        ...
        recs = ai_plan.get("recommendations", [])
        if feasibility:
            if feasibility.get("feasibility") == "unrealistic":
                recs.append(f"⚠️ 目标提醒：{feasibility.get('health_note', '')}")
            elif feasibility.get("feasibility") == "challenging":
        ...
        return {
            "motivation_quote": ai_plan.get("motivation_quote", "坚持就是胜利！"),
            "weather_impact": weather_note or "请根据实际天气调整训练安排",
            "training_split": f"今日训练: {muscle_str}" if not is_rest_day else "今日休息",
            "split_day": muscle_str,
            "is_rest_day": is_rest_day,
            "warmup": ai_plan.get("warmup", WARMUP_OPTIONS[0]),
            "workout_groups": ai_plan.get("workout_groups", []),
            "cooldown": ai_plan.get("cooldown", COOLDOWN_OPTIONS[0]),
            "meal_plan": meal_plan,
            "calorie_summary": {
                "bmr": targets["bmr"],
                "tdee": targets["tdee"],
            ...
            "progress_info": {
                ...
                "total_days": cycle_info.get("total_days", 28),
                "progress_pct": cycle_info.get("progress_pct", 0),
                "predicted_weight_change": feasibility.get("weekly_change_needed", 0) * cycle_info.get("current_week", 1),
            },
            "feasibility": feasibility,
        }
